...
public class knapsack {
    ...
    public static int getMaxValue(int[] val, int[] weight, int W) {
        ...
        for (int i = 0; i < val.length; i++) {
            rate[i][0] = i; // storing index on column 1st
            // type casting below is so important or the value can become 0
            rate[i][1] = val[i] / (double) weight[i]; // storing rate on column 2nd
        }
